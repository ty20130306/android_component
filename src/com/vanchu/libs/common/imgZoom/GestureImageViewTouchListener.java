 ...
 */
package com.vanchu.libs.common.imgZoom;
...
//				}
				...
				if (scaledHeight <= canvasHeight) {
					...
					float zth	= fitScaleHorizontal / currentScale;
					float ztv	= fitScaleVertical / currentScale;
					zoomTo		= zth < ztv	? zth : ztv;
					zoomAnimation.setTouchX(image.getCenterX());
					zoomAnimation.setTouchY(image.getCenterY());
				}
			} else {
...
//		}
	...
	public boolean isInZoom() {
		int scaledHeight = image.getScaledHeight();
		int scaledWidth = image.getScaledWidth();
		if (scaledHeight > canvasHeight || scaledWidth >canvasWidth) {
			return true;
		}
